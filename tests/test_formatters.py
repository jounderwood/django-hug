...
def test_formatter_decorator(decorator, get_function, is_valid, attr_name):
    ...
    for fn in (fn_1, fn_2, fn_3, fn_4, fn_5, fn_6):
        assert is_valid(fn)
        assert hasattr(fn, attr_name)

    formatters = get_function()

    assert formatters["application/json"] == fn_3
    assert formatters["application/msgpack"] == fn_5
    assert formatters["application/x-msgpack"] == fn_5
    assert formatters["application/bson"] == fn_6
    assert formatters["application/vnd.bson"] == fn_6
